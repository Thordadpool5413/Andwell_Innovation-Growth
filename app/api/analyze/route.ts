...
function crawlMaxPagesLimit() {
  const requested = Number(process.env.CRAWL_MAX_PAGES_PER_SITE || FULL_SCAN_MAX_PAGES_PER_SITE);
  if (!Number.isFinite(requested)) return FULL_SCAN_MAX_PAGES_PER_SITE;
  return Math.max(4, Math.min(60, Math.floor(requested)));
}
