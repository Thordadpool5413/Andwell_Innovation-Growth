...
import { Badge, Panel, ExpandableSection, SectionGroup, EmptyState } from '../Shared';
...
interface BattlecardCardProps {
  analysis: IntelligenceReport['analyses'][number];
  claims: ReturnType<typeof categorizeClaims>;
  approved: ReturnType<typeof filterApprovedClaims>;
  plan: ReturnType<typeof generateCoachingPlan>;
  copiedId: string | null;
  onCopy: (id: string, text: string) => void;
}

const BattlecardCard = memo(function BattlecardCard({ analysis, claims, approved, plan, copiedId, onCopy }: BattlecardCardProps) {
  ...
  return (
    <article className="battleCard upgradedBattle hover-card" key={analysis.id} aria-labelledby={`battlecard-title-${analysis.id}`}>
      <div className="row spread" style={{ marginBottom: '8px' }}>
        <h3 id={`battlecard-title-${analysis.id}`} className="text-heading" style={{ margin: 0 }}>{analysis.name}</h3>
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }} role="group" aria-label={`Battlecard meta: ${phase} phase, ${analysis.aiEnhanced ? 'AI enhanced' : analysis.score.threatLevel} threat level`}>
          ...
          <button
            className={`btn btn-sm ${copiedId === analysis.id ? 'copied' : ''}`}
            onClick={() => onCopy(analysis.id, copyText)}
            onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && onCopy(analysis.id, copyText)}
            title="Copy battlecard to clipboard"
            aria-label={`Copy ${analysis.name} battlecard to clipboard`}
      ...
      <ExpandableSection title="Sales positioning" defaultOpen>
        <section role="region" aria-label="Sales positioning for this competitor">
          <div className="battleSection">
            <strong>Lead with</strong>
            <div className="tagCloud" style={{ marginTop: '4px' }} role="list">
      ...
      <ExpandableSection
        ...
        defaultOpen={false}
      >
        <section role="region" aria-label="Referral questions for this competitor">
          {battlecards.length > 0 ? (
            <div className="list-grid" role="list">
              {battlecards.slice(0, 4).map((bc, i) => (
                <div key={i} className="list-card hover-card" role="listitem">
            ...
            <div className="list-grid-sm" role="list">
          ...
          <div className="battleSection" style={{ marginTop: '12px' }}>
            ...
            <div className="list-grid-sm" style={{ marginTop: '6px' }} role="list">
              {[
                ...
                <p key={i} className="text-small" style={{ margin: 0, color: 'var(--color-text-tertiary)' }} role="listitem">{q}</p>
              ))}
            </div>
          </div>
        </section>
      ...
      <ExpandableSection
        ...
        defaultOpen={false}
      >
        <section role="region" aria-label="Objection responses and common hospice objections">
          {hasExtraction && objections.length > 0 && (
            <div className="list-grid" style={{ marginBottom: '12px' }} role="list" aria-label="AI-extracted objection responses">
          ...
          <div className="list-grid" role="list">
            {objectionLibrary.slice(0, 4).map((obj, i) => (
              <div key={i} className="status-card status-card--warning hover-card" role="listitem">
      ...
      <ExpandableSection
        ...
        defaultOpen={false}
      >
        <section role="region" aria-label="Approved claims for this competitor">
          {approved.length > 0 ? (
            <div className="list-grid-sm" role="list">
              {approved.slice(0, 6).map((c, i) => (
                <div key={i} className="status-card status-card--success" role="listitem">
      ...
      <ExpandableSection title="Pre-call coaching plan" defaultOpen={false}>
        <section role="region" aria-label="Pre-call coaching plan for this competitor">
          ...
          {plan.doNotSay.length > 0 && (
            <div style={{ marginTop: '10px' }}>
              ...
              <div className="row" style={{ gap: '6px', flexWrap: 'wrap', marginTop: '6px' }} role="list" aria-label="Phrases to avoid">
                {plan.doNotSay.map((d, i) => <div key={i} role="listitem"><Badge tone="red">{d}</Badge></div>)}
              </div>
            </div>
          )}
        </section>
      ...
      <ExpandableSection title="Suggested next step" defaultOpen={false}>
        <section role="region" aria-label="Suggested next step and commitment questions">
      ...
      <ExpandableSection title="Clinical signal reminder" defaultOpen={false}>
        <section role="region" aria-label="Clinical signals to listen for">
          ...
          <div className="tagCloud" style={{ marginTop: '4px' }} role="list">
            {clinicalChecklist.map((item) => (
              <span key={item.id} title={item.help} style={{ borderColor: 'var(--color-info)', color: 'var(--color-info)', cursor: 'help' }} role="listitem">{item.label}</span>
            ))}
          </div>
        </section>
      </ExpandableSection>
      ...
      >
        <section role="region" aria-label="Review flags and items needing attention">
          {(analysis.score.needsReview || []).length ? (
            <div className="battleSection">
              <strong>Items needing review</strong>
              <div className="tagCloud" style={{ marginTop: '4px' }} role="list">
                {(analysis.score.needsReview || []).map((item) => (
                  <span key={item} style={{ borderColor: 'var(--color-danger)', color: 'var(--color-danger)' }} role="listitem">{item}</span>
                ))}
              </div>
            </div>
          ) : (
      ...
      <div className="notice" style={{ fontSize: '13px', marginTop: '8px' }} role="complementary" aria-label="Field rule">
...
export function Battlecards({ currentReport, onRunScan }: { currentReport: IntelligenceReport | null; onRunScan?: () => void }) {
  ...
  const coachingPlans = useMemo(() => {
    if (!currentReport || !analyses.length) return {};
    const plans: Record<string, ReturnType<typeof generateCoachingPlan>> = {};
    for (const analysis of analyses) {
      plans[analysis.id] = generateCoachingPlan(analysis.name, currentReport);
    }
    return plans;
  ...
  if (!currentReport) {
    return (
      <div className="card" style={{ textAlign: 'center', padding: '40px 32px', borderStyle: 'dashed', marginBottom: '0' }}>
        <FileText size={48} style={{ opacity: 0.3, marginBottom: '16px', margin: '0 auto 16px' }} />
        <p style={{ margin: '0 0 8px', fontSize: '15px', fontWeight: 700 }}>No field enablement cards yet</p>
        <p className="text-small" style={{ color: 'var(--color-text-tertiary)', margin: '0 0 16px', maxWidth: '400px', marginInline: 'auto' }}>Generate conversation playbooks for your field team after running a competitive scan. Each card includes positioning, discovery questions, objection responses, and coaching plans.</p>
        {onRunScan && <button className="btn primary" onClick={onRunScan}>Run Competitive Scan →</button>}
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <Panel title="Filters">
        <label className="row" style={{ gap: '6px', cursor: 'pointer' }}>
          <input
            ...
            aria-label="Show only competitors with approved claims"
          />
      ...
      {analyses.length === 0 ? (
        <EmptyState
          icon="💬"
          title="No field enablement cards match"
          description="No competitors match the current filter."
          action={approvedOnly && <button className="btn" onClick={() => setApprovedOnly(false)}>Clear filter</button>}
        />
      ) : (
        <div className="grid cols2">
          {analyses.map((analysis) => {
            ...
            const plan = coachingPlans[analysis.id];
            if (!plan) return null;
            return (
              <BattlecardCard
                key={analysis.id}
